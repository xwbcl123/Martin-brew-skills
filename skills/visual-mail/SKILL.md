...
2. Else: pick from the workspace brand pool if one exists (for example `40 Resources/brand-styles/*.brand_guideline.md`) by content tone.
...
Apply visual-brief principles: convert the report into a compact, reader-facing single-page HTML summary.
...
- Footer: use the configured brand footer if provided; otherwise use `Martin Design © All Rights Reserved`.

For E2E tests, write to `artifacts/e2e/assets/viz/` unless the caller provides a task/session-specific output root.
...
For E2E tests, write to `artifacts/e2e/assets/img/` unless the caller provides a task/session-specific output root.
...
For E2E tests, write to `artifacts/e2e/emails/` unless the caller provides a task/session-specific output root.
