...
**目标长度**：四个维度正文合计约 **500–1,000 字**（中文字数）。

- 若原文 chunk 内容充实（目标 5K–10K 字符量级），摘要不应过薄，避免变成标题注释式输出
- 每个维度应有实质内容，核心论点须有 2–3 条支撑证据
- 压缩率以"信息密度充足、不显得过薄"为主目标；压缩率约 1/3 仅作风格参考，不作硬验收线
- 若原文 chunk 较短（< 2,000 字符），允许摘要低于 500 字，但各维度仍需完整呈现
